...
const SOLANA_WINTERNITZ_SIGNATURE_LEN = 896;
const LEGACY_BROWSER_WOTS_SIGNATURE_LEN = 512;
...
export function createAdvancePqcWalletInstruction(
  ...
  if (
    params.signature.length !== SOLANA_WINTERNITZ_SIGNATURE_LEN &&
    params.signature.length !== LEGACY_BROWSER_WOTS_SIGNATURE_LEN
  ) {
    throw new Error(
      `PQC wallet advance signature must be ${SOLANA_WINTERNITZ_SIGNATURE_LEN} or ${LEGACY_BROWSER_WOTS_SIGNATURE_LEN} bytes, got ${params.signature.length}.`,
    );
  }
